...
const intersectionsLineAndEllipse = (A, B) => {
    ...
    if (isEqual(p1Normalized.x - p2Normalized.x, 0)) {
        ...
        const vDelta = Math.pow(b, 2) - (Math.pow(x, 2) * Math.pow(b, 2)) / Math.pow(a, 2);
        if (vDelta < 0)
            return [];
        else if (vDelta === 0) {
            ...
            const y1 = Math.sqrt((Math.pow(b, 2) * (Math.pow(a, 2) - Math.pow(x, 2))) / Math.pow(a, 2));
            const y2 = -y1;
            return [
                { x, y: y1 },
                { x, y: y2 },
    ...
    const alpha = Math.pow(a, 2) * Math.pow(angular, 2) + Math.pow(b, 2);
    const beta = 2 * Math.pow(a, 2) * (angular * linear);
    const gamma = Math.pow(a, 2) * (Math.pow(linear, 2) - Math.pow(b, 2));
    const delta = Math.pow(beta, 2) - 4 * alpha * gamma;
    if (delta < 0)
        return [];
    else if (delta === 0) {
        const x = -(Math.pow(beta, 2)) / (2 * alpha);
        const y = lineY(x);
